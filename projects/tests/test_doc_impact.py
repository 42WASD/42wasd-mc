...
def _kubectl(*args: str) -> tuple[int, str]:
    # Uses the ambient KUBECONFIG when readable; otherwise retries via sudo
    # with the var passed as a command prefix (sudo -E is rejected when the
    # caller's environment can't be preserved, e.g. non-tty contexts).
    rc, out = _run(["kubectl", *args], timeout=45)
    if rc == 0:
        return rc, out
    kc = os.environ.get("KUBECONFIG", "")
    if kc:
        return _run(["sudo", f"KUBECONFIG={kc}", "kubectl", *args], timeout=45)
    return _run(["sudo", "kubectl", *args], timeout=45)
...
def probe_kubectl_json(c: dict) -> tuple[bool, str]:
    ...
    if rc != 0:
        # No reachable cluster in this environment -> skip, don't fail (CI,
        # machines without the cluster). A wrong claim still fails later when
        # run against the real cluster.
        return None, f"kubectl get {res[0]} rc={rc}: {out[:100]}"
    try:
