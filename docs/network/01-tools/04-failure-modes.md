...
```text
Pod phase = Running → transfer player
```
...
Reserve capacity for the full party. Do not independently random-route members.

## Invite target changed during launcher restart

A pending invite should contain:
...
Treat uploaded worlds as untrusted. Perform:
...
```text
start → timeout → mark DEGRADED → return player to lobby → surface operator alert
```
...
A serious authentication/security failure because backend servers are normally in offline mode behind the proxy.
