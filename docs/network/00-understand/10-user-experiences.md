...
The player may remain in the lobby with a message like `"Starting Floating Kingdom…"`. Do **not** transfer to a TCP port merely because the Pod exists.
...
Steve is running the vanilla runtime. Alex is in `fantasy-1.20.1-forge`.
...
Nakama stores pending invite:
  ...
  expires_at=...
        ↓
Steve sees: "This world requires Fantasy Runtime."
[Install / Launch Runtime]
        ↓
Modrinth Server Project installs/updates required content
        ↓
launches the correct Minecraft runtime into the public server
        ↓
