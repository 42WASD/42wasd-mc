...
The architecture deliberately separates concerns into seven layers.

## 2.1 ENTRY — How does the player reach the network?

**Recommended:** `mc-router (optional) → Velocity`
...
Velocity is selected because this architecture values the **usable Minecraft plugin ecosystem** more than minimizing proxy process size.
...
**Recommended container foundation:** `itzg/minecraft-server`
...
**Recommended on Velocity:** ViaVersion, ViaBackwards.

Use protocol translation only for runtime classes designed to tolerate version translation. Do not treat it as a substitute for mod compatibility testing.

## 2.7 CLIENT RUNTIME — Does the player have the required game + loader + mods?

**Recommended public onboarding:** Modrinth Server Projects.

**Recommended pack authoring/CI option:** packwiz.
