...
- **Despawn ranges**: hard 128 → **72** (56 for water mobs) — far fewer entities
  tracked; players rarely notice.
...
- Increase **hopper-check-interval** / reduce `HopperEvent` moves where possible.
...
## 3. Layer 3 — Performance tooling (spark, plugins)
...
| **FarmLimiter** | Cap entity counts / mob-farm growth |

If a plugin is expensive (per the runbook: e.g. stacker/living stacked mobs)
**remove it** — a single bad plugin is often 40% of a tick. Run **Spark** to find
...
- **Velocity** (BungeeCord successor) as an L7 proxy to split player load across
  ...
  (see the game-edge decision). Does not change per-server tick; adds a layer that
  can bundle player-facing latency display (proxy-only ping).
